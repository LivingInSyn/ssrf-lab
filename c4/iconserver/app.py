...
class UserIcon(Resource):
    ...
    def put(self, user_id):
        ...
        if user_id not in users:
            ...
            users[user_id]['icon_path'] = args['icon_path']
        # require icon_path to end in .png
        if not args['icon_path'].lower().endswith('.png'):
            return {'error': 'Bad file extension'}, 401
        # pretend to be node or any other null byte processing lib
        args['icon_path'] = args['icon_path'].split("\x00")[0]
        print('icon path: ', args['icon_path'])
        # make a request to secretserver
